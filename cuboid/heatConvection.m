...
global tcell_type; global isHuman;global isHeat;
 %热对流的概率交换过程
      for x1=2:length-1
        for y1=2:width-1
            for z1=2:height-1
...
%                  if(x1<humanX_start-1||x1>humanX_end+1||y1<humanY_start-1||y1>humanY_end+1||z1<humanZ_start-1||z1>humanZ_end+1)
                    if(tcell_type(x1,y1,z1)==isHuman ||tcell_type(x1+1,y1,z1)==isHuman||tcell_type(x1-1,y1,z1)==isHuman||...
                     ...
                     tcell_type(x1,y1,z1)==isHeat ||tcell_type(x1+1,y1,z1)==isHeat||tcell_type(x1-1,y1,z1)==isHeat||...
                         tcell_type(x1,y1+1,z1)==isHeat||tcell_type(x1,y1-1,z1)==isHeat||...
                         tcell_type(x1,y1,z1+1)==isHeat||tcell_type(x1,y1,z1-1)==isHeat )
                      continue;
                    else
                    %概率和边界条件
